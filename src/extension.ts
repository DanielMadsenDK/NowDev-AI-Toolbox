...
function getArtifactStatePath(workspaceRoot: string): string | undefined {
...
function buildArtifactHookManifest(): string {
...
function buildArtifactMergeHookScript(): string {
...
function buildArtifactMergeShellWrapper(): string {
        return '#!/usr/bin/env bash\nnode .vscode/nowdev-ai-hooks/merge-artifact-manifest.js\n';
}

function buildArtifactMergePowerShellWrapper(): string {
...
async function executeFirstAvailable(commandsToTry: string[], unavailableMessage: string): Promise<boolean> {
    const commands = await vscode.commands.getCommands(true);

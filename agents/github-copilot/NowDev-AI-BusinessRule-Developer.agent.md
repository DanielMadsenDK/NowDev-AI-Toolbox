...
<workflow>
1. **Context Sync**: Read `.vscode/nowdev-ai-config.json`, then read the artifact state file at `artifactState.path` if it exists to discover artifacts created by sibling agents — especially Script Include class names and method signatures. If only `memoryLocation` exists, treat it as optional legacy context.
...
In Fluent SDK projects, **BusinessRule `script` accepts function references** — use ES module `import`/`export` directly instead of inline strings. Verify the current pattern with `now-sdk explain businessrule-api --format raw` and `now-sdk explain module-guide --format raw`.
...
Follow `agents/skills/servicenow-artifact-state/SKILL.md`. Read the workspace artifact state before implementation, read dependency source files for exact Script Include/table signatures, and end with a final `Artifact Manifest` JSON block.
